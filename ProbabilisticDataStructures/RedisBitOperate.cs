...
namespace BloomFilter.Redis
{
    ...
    public class RedisBitOperate
    {
        ...
        public RedisResult GetBitField(string redisKey, uint bitLength, uint offset)
        {
            object[] args = new object[] { redisKey, "GET", string.Format("u{0}", bitLength), string.Format("#{0}", offset) };
            return Database().Execute("BITFIELD", args);
        }

        public RedisResult IncrBitField(string redisKey, uint bitLength, uint offset, int delta)
        {
            if (bitLength == 1)
            {
                if (delta < 0)
        ...
        public RedisResult SetBitField(string redisKey, uint bitLength, uint offset, int value)
        {
            object[] args = new object[] { redisKey, "SET", string.Format("u{0}", bitLength), string.Format("#{0}", offset), value};
            return Database().Execute("BITFIELD", args);
        }
